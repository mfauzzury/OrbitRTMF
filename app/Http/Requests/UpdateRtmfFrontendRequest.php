...
class UpdateRtmfFrontendRequest extends BaseFormRequest
{
    ...
    {
        $id = $this->route('rtmf_frontend');

        return [
            'spec_id' => [
                'sometimes', 'required', 'string', 'max:64',
                Rule::unique('rtmf_frontends', 'spec_id')->ignore($id),
            ],
            'module_id' => 'sometimes|required|integer|exists:rtmf_modules,id',
